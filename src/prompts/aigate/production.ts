/**
 * AI Gate Agent — Production prompt (v2)
 *
 * Reviews triaged questionid entries one at a time with full survey context.
 * Proposes targeted mutations to structural classifications before table generation.
 *
 * Prompt structure modeled on the VerificationAgent alternative prompt:
 * mission → input reality → default posture → per-rule decision guidance →
 * evidence hierarchy → decision framework → hard bounds → confidence calibration
 */

export const AIGATE_AGENT_INSTRUCTIONS_PRODUCTION = `
<mission>
