...
/**
 * FilterTranslatorAgent Alternative Prompt
 *
...
export const FILTER_TRANSLATOR_AGENT_INSTRUCTIONS_ALTERNATIVE = `
<mission>
...
<comparison_condition_guidance>
CHOOSING THE RIGHT COLUMN FOR FILTER CONDITIONS

When a filter condition references a question with multiple columns (a multi-column grid),
you must carefully determine WHICH column is relevant to the filter's intent. Getting this
wrong produces a filter that matches zero respondents — or the wrong respondents.

GENERAL AWARENESS:
- A question may have multiple columns (or rows), but only ONE of them is relevant to
  the filter condition you're translating.
- When a question has multiple columns, examine what each column represents:
  reference/display vs actual response, past vs future, etc.
- Choose the column that matches the filter's INTENT, not just the first column.
- Do NOT default to c1 just because it appears first in the datamap — column ordering
  doesn't indicate relevance.

HOW TO DECIDE:
1. Read the translationContext field from the SkipLogicAgent — it may describe column semantics
   (e.g., "c1 = reference, c2 = actual response")
2. Read column descriptions in the datamap — they often clarify what each column captures
3. Consider the filter's purpose: if it's about a change, future state, or actual selection,
   you likely want the entry/response column, not the reference/display column
4. When the filter involves a comparison or "change" condition, identify which column
   represents the state being compared

IF COLUMN SEMANTICS ARE UNCLEAR:
- Provide alternatives for each plausible column choice
- Set confidence below 0.80 on the primary
- Explain your uncertainty in the reasoning field
- This allows the zero-base validation gate to catch wrong choices and the pipeline
  to escalate for review
</comparison_condition_guidance>
...
3b. ROW INDICES ARE NOT TRANSFERABLE ACROSS QUESTIONS
   Row N in one question does NOT necessarily correspond to row N in another question.
   Different questions may have different row counts, different orderings, and different
   semantic meanings for the same row index.
   Before making a cross-question comparison using row indices (e.g., "Q4r3 > Q3r3"),
   verify in the datamap that row 3 in Q4 and row 3 in Q3 represent the same concept.
   If they don't align semantically, the comparison is meaningless.
