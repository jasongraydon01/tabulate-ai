...
const WINCROSS_TABLE_TITLE_MAX_LINES = 4;
// WinCross title-width settings are not encoded reliably in our exported profile,
// so use a conservative default to avoid desktop line-overflow errors.
const WINCROSS_TABLE_TITLE_ESTIMATED_WRAP_WIDTH = 30;
...
export function serializeWinCrossJob(
  ...
  if (profile.sigFooterLines.length > 0) {
    for (const line of profile.sigFooterLines) lines.push(line);
  } else {
  ...
  for (const table of tables) {
    ...
    const titleLayout = layoutWinCrossTableTitle(
      ...
      resolveWinCrossTableTitleWrapWidth(profile),
    );
    ...
    if (titleLayout.wrapped) {
      tableNotes.push(`Wrapped table title across ${titleLayout.lineBreakCount + 1} WinCross title lines.`);
    }
    if (titleLayout.truncated) {
      tableWarnings.push(
        `Table ${table.tableId} title exceeded ${WINCROSS_TABLE_TITLE_MAX_LINES} WinCross title lines and was truncated.`,
      );
      tableNotes.push('Truncated table title to fit WinCross title limits.');
    }
    ...
    const nativeAfLine = indexedTable.idxFilter ? null : deriveNativeAfLine(bodyRows);
    ...
    if (existingAnchor) {
      ...
      if (derivedUseLine) {
        ...
        anchors.push(anchor);
        if (nativeAfLine) {
          lines.push(nativeAfLine);
          afCount += 1;
          usedAf = true;
          emittedNativeAfLine = true;
          afStrategy = 'native_single_variable_stat';
          tableNotes.push(`Applied native AF strategy: ${nativeAfLine}.`);
        }
    ...
    if (!nativeAfLine && additionalFilter.length > 0) {
  ...
  if (profile.titleLines.length > 0) {
    for (const line of profile.titleLines) lines.push(line);
  } else {
...
function isSafeWinCrossTitleCandidate(value: string): boolean {
  ...
  return estimateWrappedLineCount(normalized, WINCROSS_TABLE_TITLE_ESTIMATED_WRAP_WIDTH) <= WINCROSS_TABLE_TITLE_MAX_LINES;
}

function layoutWinCrossTableTitle(title: string, wrapWidth = WINCROSS_TABLE_TITLE_ESTIMATED_WRAP_WIDTH): WinCrossTitleLayout {
  ...
  while (remaining.length > 0 && lines.length < WINCROSS_TABLE_TITLE_MAX_LINES) {
    const currentPrefixLength = lines.length === 0
      ? 0
      : lines.reduce((sum, line) => sum + line.length, 0) + lines.length; // existing pipes
    const remainingBudget = WINCROSS_TABLE_TITLE_MAX_CHARS - currentPrefixLength;
    ...
    const widthBudget = Math.max(1, Math.min(remainingBudget, wrapWidth));

    if (remaining.length <= widthBudget) {
      lines.push(remaining);
      remaining = '';
      break;
    }

    if (lines.length === WINCROSS_TABLE_TITLE_MAX_LINES - 1) {
      lines.push(appendTitleEllipsisWithinLimit(remaining, widthBudget));
      remaining = '';
      truncated = true;
      break;
    }
    ...
    wrapped = true;
    const splitIndex = findTitleSplitIndex(remaining, widthBudget);
  ...
  if (remaining.length > 0) {
    ...
    const usedBeforeLast = lastLineIndex <= 0
      ? 0
      : lines.slice(0, lastLineIndex).reduce((sum, line) => sum + line.length, 0) + lastLineIndex;
    const lastLineBudget = Math.max(1, WINCROSS_TABLE_TITLE_MAX_CHARS - usedBeforeLast);
    lines[lastLineIndex] = appendTitleEllipsisWithinLimit(lines[lastLineIndex] ?? '', lastLineBudget);
  }

  const serializedLine = (lines.length > 0 ? lines : ['Table']).join('|');

  return {
    line: serializedLine,
    lineBreakCount: (serializedLine.match(/\|/g) ?? []).length,
    wrapped: wrapped || serializedLine.includes('|'),
    truncated,
  };
}

function resolveWinCrossTableTitleWrapWidth(profile: WinCrossPreferenceProfile): number {
  const vector = profile.numericPreferenceVector?.trim() ?? '';
  if (!vector) {
    return WINCROSS_TABLE_TITLE_ESTIMATED_WRAP_WIDTH;
  }

  const tokens = vector.split(',').map((token) => token.trim());
  const rawWidth = Number(tokens[1]);
  if (Number.isFinite(rawWidth) && rawWidth > 0) {
    return Math.max(20, Math.min(rawWidth, WINCROSS_TABLE_TITLE_ESTIMATED_WRAP_WIDTH));
  }
...
function formatHeaderLabel(label: string, indentValue: unknown, leadingSpaces: number | null): string {
  ...
  return `${prefix}${applyRowIndent(label, indentValue)}`;
}
...
function formatBannerLabelOnlyLine(label: string, memberSuffix: string | null, memberPrefix = ''): string {
  if (memberSuffix) {
    return `${memberPrefix}${label}^${memberSuffix}`;
  }
