...
- **Tier B banner-extension recompute — implemented for one appended group.** When the user asks to add a new cut across the full tab set, treat that as a run-level extension, not a table-card artifact. The analysis workspace now offers an explicit derived-run action that preflights one appended banner group, shows a structured proposal card from a sanitized job read model, requires button confirmation, queues a child run through the worker, tracks queued/running/terminal/expired status after refresh, and hands off into the derived run's analysis workspace when ready. The original run stays immutable; the child run carries lineage metadata to the parent run and compute job.
- **Approval + quality gate policy for analysis-triggered compute — implemented for Tier B.** Clean preflight plus explicit user confirmation is the approval event for one-group banner extension. Unsafe preflight jobs persist `needs_clarification` / review flags and cannot be confirmed. Persisted cancel/reject is available for proposed, blocked, queued, and running jobs. Raw compute-job documents are internal-only; the browser sees a client-safe projection with no R2 keys or frozen artifacts. We do not route the user back through the processor-facing HITL review step just to add a requested cut.
- **Next compute-lane product slice: Tier A single-table derivations.** Add small-scope, compute-backed extensions to an existing run: one table, or a very small set of related tables, with one added cut, NET, or derived view. These outputs must be real computed artifacts, not agent-authored arithmetic. They should persist as sibling derived artifacts on top of the source run rather than mutating `results/tables.json` in place. Lineage should be explicit: source run, source table, derivation type, and who requested it.
- **Still deferred.** Typed confirmation, clarification repair against an existing job, multi-group recompute, old-group editing, broad derived-run history, and promotion of derived outputs into delivery-grade artifacts remain usage-driven later slices.
