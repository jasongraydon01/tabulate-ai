...
- **Next compute-lane product slice: Tier A table-scoped derivations.** Add small-scope, compute-backed extensions to an existing run: one table, or a very small set of related tables, with one added cut, NET, or derived view. This is the lane for cases where the user or agent is asking for a table-level computed follow-up instead of a whole derived run. These outputs must be real computed artifacts, not agent-authored arithmetic. They should persist separately from canonical parent-run artifacts, with explicit lineage to source run, source table, derivation type, and requester.
