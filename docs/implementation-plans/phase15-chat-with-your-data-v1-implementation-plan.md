...
- **Current compute-lane product slice: Tier A table-scoped derivations.** Add small-scope, compute-backed extensions to an existing run, starting with one selected source table. Bucket 1 row roll-ups now work end to end for artifact-safe same-variable exclusive sums: the agent uses `proposeRowRollup`, backend validation creates durable proposal jobs only for valid candidates, confirmation remains button-only, the worker creates `computed_derivation` artifacts rather than child runs, and the same conversation can fetch and interpret the computed table. The V1 sequence remains: row roll-ups first, selected-table cuts second, non-roll-up derived tables third. Roll-ups collapse selected existing rows while preserving the table's analytical meaning. TabulateAI recognizes but blocks unsupported mechanisms such as respondent-level any-of NETs and metric row aggregation until their safe compute paths exist, and it must not hand-add displayed percentages when selected rows can overlap. Selected-table cuts are the next workflow for creating new respondent groups/columns. Non-roll-up derived tables remain V1 work for KPI side-by-side tables, tables assembled across questions, composites, intersections, and table-type transformations.
- **Compute reuse optimization and UX smoothness — v1 polish after core Tier A.** The current lane is conservative and may recompute or re-send settled parent-run context through agents even when the derived request does not change those decisions. Before calling the compute lane fully smooth, add reuse/caching around stable parent artifacts and unchanged semantic decisions, while preserving frozen confirmed inputs, worker-queued execution, and fresh validation for the newly requested cut, roll-up, or derived table definition. Also improve queued/running derived-table feedback and the auto-continuation experience so the UI clearly shows progress while compute is underway and renders/interprets the computed artifact promptly once available. After that polish, design multi-table row roll-ups for small related table sets; keep broad all-tab changes in the derived-run lane.
