...
Direction: when a `rowFilter` is provided through the tool call, the rendered table should sort matched rows to the top. The sorting hierarchy is: (1) if the filter matches a single row, that row anchors at the top; (2) if the filter matches multiple rows, all matched rows anchor at the top as a group, but within that matched set they preserve the default answer-option ordering from the table; (3) unmatched rows follow below in their original order. The matched rows should also receive a visual highlight (left border accent + subtle background tint) so the user can see what the model focused on without needing to check the Details panel. This is already partially implemented in the grounding layer's row prioritization logic and the card's row highlight rendering.
