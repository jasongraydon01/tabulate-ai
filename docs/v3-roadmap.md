...
- Grounded lookup tools: `searchRunCatalog`, `fetchTable`, `getQuestionContext`, `listBannerCuts`
...
- **Track B — Prompt intentionality + analytical depth (deferred, design first).** Four sub-areas: explicit workflow prompting (request classification, scope check, acknowledgment-before-work, internal goal-setting); agent context optimality (what survives from prior turns — tool outputs are currently stripped entirely on subsequent turns, leaving the agent blind to its own prior evidence); tool-surface review (scratchpad vs reasoning summaries, fetchTable scope/renders, audit against real traces before adding more); and capability expansion (cut building, NET generation, derived tables). Crosses into Slice 6 (compaction policy) and Slice 7 (compute-lane checkpoint). No code changes until design notes exist and have been pressure-tested against real session transcripts; production prompt stays frozen, workflow experiments land on alternative first.
